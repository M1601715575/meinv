...
/* sobel*/
	...
	Sobel(image, grad_x, CV_16S, 1, 0, 3, 1, 1, BORDER_DEFAULT ); /*求x方向梯度*/
	...
	addWeighted( abs_grad_x, 0.5, abs_grad_y, 0.5, 0, sob1 );  /*两个数组的加权和*/
...
/*Laplacian*/
	...
	Laplacian(image, laps1, CV_16S, 1, 1, 0, BORDER_DEFAULT);
...
/*pretitt*/
	...
	Mat kernel_x = (Mat_<int>(3, 3) << -1, -1, -1, 0, 0, 0, 1, 1, 1); /*3*3矩阵*/

	filter2D(image, imgx, -1, kernel_x, Point(-1, -1), 0, 0);/*卷积和*/
